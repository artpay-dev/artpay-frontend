...
import AwaitingBankTransfer from "./components/awaitingbanktransfer/AwaitingBankTransfer.tsx";
...
const CdsPayments = () => {
  ...
  const paymentStage = useMemo(() => {
    ...
    // Check if Santander payment is awaiting bank transfer
    if (
      paymentStatus === PAYMENT_STATUS.PROCESSING &&
      paymentMethod === PAYMENT_METHODS.SANTANDER &&
      orderNote?.includes("In attesa dell'accredito")
    ) {
    ...
    return 'confirmation';
  }, [paymentMethod, paymentStatus, orderNote]);
  ...
  const renderContent = () => {
    ...
    if (!order || loading) {
      switch (paymentStage) {
        ...
        case 'awaiting_transfer':
          return <AwaitingBankTransfer order={order as Order} isLoading={true} />;
    ...
    switch (paymentStage) {
      ...
      case 'awaiting_transfer':
        return <AwaitingBankTransfer order={order} isLoading={false} />;
