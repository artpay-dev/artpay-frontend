...
const SantanderFlow = ({ isLoading, order }: SantanderFlowProps) => {
  ...
  const [isLoanObtained, setIsLoanObtained] = useState(false);
  const data = useData();
                  ...
                  <div className={"space-y-4"}>
                    <div className={"flex gap-6 items-center "}>
                      ...
                      <h3 className={"text-lg leading-[125%] text-tertiary text-balance"}>
                        Complimenti il tuo lotto è pronto per{" "}
                        {orderNote == "" ? <span>richiedere il prestito</span> : <span>completare il pagamento</span>}
                      </h3>
                    </div>
                    ...
                    {!isLoanObtained ? (
                      <>
                        ...
                        <div className={"space-y-6 flex flex-col"}>
                          <button
                            className={"text-primary underline text-start cursor-pointer"}
                            onClick={() => {
                              setIsLoanObtained(true)
                            }}>
                            Ho già ottenuto il prestito
                          </button>
                        </div>
                      </>
                    ) : (
