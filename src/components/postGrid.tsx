...
const transformPost = (post: ContentfulPost): PostListItem => {
  ...
  const slug = post.slug || post.sys?.id || title.toLowerCase().replace(/\s+/g, "-");
        ...
        {post.cover && (
          <Link to={`/guide/${post.slug}`}>
            <img
              ...
              className="object-cover w-full h-full rounded-[4px] aspect-square"
              loading="lazy"
            />
          </Link>
        )}
...
export function PostGrid() {
  ...
  return (
    <Grid px={0} container>
      <Grid xs={12} sx={{ maxWidth: "100%", overflow: "auto", py: 5, px: { xs: 0, md: 0 } }}>
