...
const ChatContent: React.FC<ChatContentProps> = ({
  ...
  const initials = (
    (userProfile?.first_name || "").substring(0, 1) + (userProfile?.last_name || "").substring(0, 1)
  ).toUpperCase();
  ...
  return (
    <Box display="flex" flexDirection="column" gap={3} sx={{ py: 3, px: { xs: 0, sm: 3 } }}>
      {messages.map((message, i) => (
        <Box
          ...
          <Box>
            {message.userMessage ? (
              <AvatarCircleText text={initials} />
            ) : (
          ...
          <Box flexGrow={1} pl={1} display="flex" flexDirection="column">
            <Box display="flex">
              <Typography variant="subtitle1" flexGrow={1}>
                {message.userMessage ? "Tu" : "Galleria"}
              </Typography>
              <Typography variant="body2" color="textSecondary">
                {formatMessageDate(message.date.toDate())}
              </Typography>
            </Box>
            <Typography variant="subtitle1" color="textSecondary" sx={{ whiteSpace: "pre-line" }}>
              <Linkify
                options={{
                  target: "_blank",
      ...
      <Box>
        <TextField
          ...
          InputProps={{
            endAdornment: (
              <IconButton disabled={sending} onClick={handleSendMessage} color="primary">
                <Send />
              </IconButton>
            ),
          }}
