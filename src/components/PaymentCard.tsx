...
const PaymentCard: React.FC<PaymentCardProps> = ({
  ...
  const {privacyChecked, updateState, isSaving, pendingOrder} = useDirectPurchase()
  ...
  return (
    <ContentCard
      ...
      <PaymentProviderCard cardTitle={paymentMethod == 'card' ? "Carta di credito" : "Klarna"} backgroundColor={providerCardStyles[paymentMethod as keyof typeof providerCardStyles]?.style }
        ...
        <div className={"space-y-4 py-6 border-t border-b border-gray-950/20"}>
          <div className="flex items-center justify-between">
            <span className={'font-semibold'}>
              {paymentMethod == 'klarna' ? 'Prezzo' : 'Prezzo opera'}
              {paymentMethod != 'klarna' && (<><br /><span className={'text-secondary text-sm font-light'}>Incluse commissioni artpay</span></>)}
            </span>
            <span>€&nbsp;
              {paymentMethod == 'klarna'
                ...
                : ((paymentIntent?.amount || 0) / 100).toLocaleString("it-IT", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
              }
            </span>
          </div>
          {paymentMethod == 'klarna' && (
